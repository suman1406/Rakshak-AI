...
revision = "0006_add_video_diag_cols"
...
def upgrade() -> None:
    ...
    # Add confidence_band if not exists
    ...
    # Add decision_authority if not exists
    if "decision_authority" not in columns:
        op.add_column(
            "video_diagnoses",
            sa.Column("decision_authority", sa.String(50), nullable=True, server_default="llm")
        )
...
def downgrade() -> None:
    bind = op.get_bind()
    columns = {column["name"] for column in sa.inspect(bind).get_columns("video_diagnoses")}
    if "decision_authority" in columns:
        op.drop_column("video_diagnoses", "decision_authority")
