...
class _NewScanState extends State<NewScanScreen> {
  final formKey = GlobalKey<FormState>(); String? fileName; String? filePath; String? uploadedVideoId; bool consent = false; bool uploading = false;
  ...
  Future<void> continueToQualityCheck() async {
    ...
    if (filePath != null) {
      try { final upload = await ApiClient.instance.uploadVideo(fieldId: 'field-north-plot', filePath: filePath!, consent: consent); uploadedVideoId = upload['video_id'] as String?; } catch (_) { /* demo flow remains available */ }
    }
    if (!mounted) return;
