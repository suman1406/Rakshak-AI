...
class _NewScanState extends State<NewScanScreen> {
  ...
  Future<void> _loadFields() async { try { final fields = await ApiClient.instance.listFields(); if (!mounted) return; setState(() { availableFields = fields; fieldId = fields.isEmpty ? null : fields.first['id'].toString(); }); } catch (error) { if (mounted) setState(() => uploadError = 'Could not load fields: $error'); } }
